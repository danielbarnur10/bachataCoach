...
function makeService(apiKey?: string): PracticeReviewService {
  if (apiKey) process.env.OPENAI_API_KEY = apiKey;
  else delete process.env.OPENAI_API_KEY;
  delete process.env.OPENAI_KEY;
  return new PracticeReviewService();
}
...
describe('PracticeReviewService — heuristic fallback (no API key)', () => {
  it('creates timestamped segments for a short clip even without an AI provider', async () => {
    const review = await makeService().reviewVideo('Short practice clip', {
      durationSeconds: 24,
    });
  ...
  it('returns all required fields', async () => {
    const review = await makeService().reviewVideo('Test', {
      durationSeconds: 60,
    });
  ...
  it('produces at least 2 segments for a very short clip', async () => {
    const review = await makeService().reviewVideo('Tiny', {
      durationSeconds: 5,
    });
...
describe('PracticeReviewService — AI path', () => {
  const aiPayload = {
    ...
    segments: [
      { startTime: 0, endTime: 5, label: 'Derecho', reason: 'Solid timing' },
    ],
  };
  ...
  it('injects userFeedback into the AI prompt as a DANCER CORRECTION block', async () => {
    ...
    await makeService('test-key').reviewVideo('Test', {
      durationSeconds: 24,
      userFeedback: 'the energy was sensual not aggressive',
    });
  ...
  it('filters segments with endTime <= startTime', async () => {
    ...
    mockFetch({
      choices: [
        {
          message: {
            content: JSON.stringify({ ...aiPayload, segments: badSegments }),
          },
        },
      ],
    });
...
describe('PracticeReviewService — chat', () => {
  ...
  it('returns the AI reply and actions', async () => {
    const aiJson = {
      reply: 'Great footwork!',
      actions: [{ type: 'setSpeed', rate: 0.75 }],
    };
  ...
  it('returns empty actions when AI sends none', async () => {
    mockFetch({
      choices: [
        { message: { content: JSON.stringify({ reply: 'ok', actions: [] }) } },
      ],
    });
  ...
  it('throws when fetch returns a non-ok status', async () => {
    ...
    await expect(makeService('test-key').chat('Hello', [])).rejects.toThrow(
      '401',
    );
  });
});
