...
export default function App() {
  ...
  async function reviewVideo(video: Video) {
    ...
    const response = await api(`/videos/${video.id}/review/chunk/1?aTime=${range.aTime}&bTime=${range.bTime}&libraryId=${encodeURIComponent(libraryId)}`);
    const data = await response.json();
    if (!response.ok || data.error) return setStatus(data.error || 'Review failed.');
    setReview({ video, data }); setStatus('Review complete.');
  }
